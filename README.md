...
### קובץ ה־CSV זהה לגיליון Data

אותן עמודות, אותו סדר ואותם ערכים — שניהם נכתבים מאותה טבלה. 63 עמודות הדמה
של הפירוק מופרדות משניהם: באקסל הן עוברות לגיליון `Decomposition` נפרד, מזוהות
לפי `symbol` ו־`period_key`, ובפלט ה־parquet הן נשארות בטבלה הראשית.

כל שדות הנתונים הפיננסיים והמחושבים נשמרים בקוד כ־`float` ונכתבים **בדיוק מלא
בשני הפורמטים**. העיגול קיים רק בתצוגת האקסל — ברירת המחדל היא שתי ספרות
(`0.00`), עם שני חריגים:

- **תרומות, משקלים ושאריות** מוצגות בשש ספרות (`0.000000`) — תרומת ROIC של 31
  נקודות בסיס הייתה נראית כאפס בפורמט הרגיל.
...
לכן תא שמוצג באקסל כ־`313.33` ייכתב ל־CSV כ־`313.3299865722656` — אותו מספר,
תצוגה שונה. פורמט xlsx שומר 16 ספרות מובהקות ולא 17, ולכן ה־CSV הוא למעשה
הצד המדויק מבין השניים; ההפרש הוא ULP בודד, בסדר גודל של ‎1e-16.

⚠️ בקריאת ה־CSV ב־pandas יש להעביר `float_precision="round_trip"`. הפרסר המהיר
שהוא ברירת המחדל של `read_csv` אינו מעוגל נכון ומחזיר כ־13 ספרות מובהקות בלבד —
סטייה שאינה קיימת בקובץ עצמו.
