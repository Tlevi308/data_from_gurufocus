...
- אין עיגול בשלב החישוב. העיגול קיים רק בתצוגה: `0.00` ברוב העמודות,
  `0.000000` בתרומות, במשקלים ובשיעורי הריבית — תרומה של 31 נקודות בסיס
  ועלות חוב של 2.7% היו מוצגות כאפס בפורמט הרגיל.
...
ב-Excel הן עוברות לגיליון `Decomposition` נפרד עם `symbol` ו-`period_key`;
ב-CSV וב-parquet הן נשארות בטבלה הראשית.
